...
export async function getSchools() {
  const url = `https://univast.faraday.africa/academia/schools/NG`;
  const config = {
    headers: {
...
export async function getFaculties(code) {
  const url = `https://univast.faraday.africa/academia/faculties/${code}`;
  const config = {
    headers: {
      "Content-Type": "application/json",
      Authorization: `Api-Key ${process.env.REACT_APP_UNIVAST_KEY}`,
