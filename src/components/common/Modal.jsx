...
const Modal = ({ visible, icon, title, message, action, cancel }) => {
  const modal = useRef();
  useEffect(() => {
    setTimeout(() => {
      gsap.fromTo(
    ...
    <>
      {visible && (
        <div className="z-50  fixed top-0 left-0 w-full h-full bg-[#000000bb] flex justify-center items-center">
          <div
            ref={modal}
            className="bg-white p-10 rounded-3xl max-w-[450px] min-w-[300px] text-center opacity-0 m-4"
          >
            ...
            {cancel && (
              <button
                onClick={() => cancel()}
                className="border border-faraday-night text-faraday-night py-3 px-4 rounded-xl w-full mt-2"
              >
                Cancel
              </button>
            )}
